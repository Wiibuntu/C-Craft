...
// Forward declarations for biome helpers
...
{
    float t = fbmNoise(x * 0.0012f + 100.0f, z * 0.0012f - 100.0f, 4, 2.0f, 0.5f);
    float h = fbmNoise(x * 0.0012f - 200.0f, z * 0.0012f + 200.0f, 4, 2.0f, 0.5f);
    float w = fbmNoise(x * 0.00055f + 999.0f, z * 0.00055f - 999.0f, 3, 2.0f, 0.5f);
...
static Biome getBiomeBase(int x, int z)
{
    float temp, humid, weird;
    getClimate((float)x, (float)z, temp, humid, weird);

    // Big-scale ocean mask. Lower threshold a bit to create larger ocean bodies.
    float ocean = oceanMask((float)x, (float)z);
    if(ocean < -0.30f)
        return BIOME_OCEAN;

    // Ridged areas -> hills / rocky
    float r = ridge2D((float)x, (float)z, 0.0010f);
    float rare = fbmNoise(x * 0.00022f + 50.0f, z * 0.00022f - 50.0f, 2, 2.0f, 0.5f);
    float rare01 = (rare + 1.0f) * 0.5f;

    // Extreme hills: still rare, but not "never"
    if(r > 0.68f && weird > 0.60f && rare01 > 0.62f)
        return BIOME_EXTREME_HILLS;

    // Rocky biome: medium ridges + high "weirdness"
    if(r > 0.58f && weird > 0.70f)
        return BIOME_ROCKY;

    // Swamp: wet + warm-ish
    if(humid > 0.76f && temp > 0.40f)
        return BIOME_SWAMP;

    // Desert: hot + dry
    if(temp > 0.72f && humid < 0.36f)
        return BIOME_DESERT;
    ...
    // Savanna: hot-ish but not as dry as desert
    if(temp > 0.62f && humid >= 0.36f && humid < 0.55f)
        return BIOME_SAVANNA;

    // Forest: wetter regions (but not swamp)
    if(humid > 0.55f)
        return BIOME_FOREST;
...
// Returns the biome at (x,z) in the same way chunk generation decides it.
// This is important so the dev overlay matches what you see (beaches, etc.).
static Biome getBiomeAt(int x, int z)
{
    Biome b = getBiomeBase(x, z);

    // Ocean stays ocean; beaches are an edge-band near sea level.
    if(b != BIOME_OCEAN && b != BIOME_DESERT && b != BIOME_EXTREME_HILLS) {
        int surfaceY = (b == BIOME_EXTREME_HILLS)
            ? getExtremeHillsBase(x, z)
            : getHeight2D(x, z, b);
...
// Backward compatible name (many call sites)
...
static BlockType surfaceTopForBiome(Biome b, int surfaceY)
{
    ...
    if(b == BIOME_ROCKY)  return BLOCK_GRAVEL;
    ...
    // Plains / forest / taiga / swamp / savanna all use grass as a readable default.
    return BLOCK_GRASS;
}
...
static BlockType surfaceFillerForBiome(Biome b, int surfaceY)
{
    ...
    if(b == BIOME_ROCKY)  return BLOCK_COBBLESTONE;
...
static bool shouldPlaceTree(Biome b, int x, int z, int surfaceY)
{
    ...
    if(!(b == BIOME_FOREST || b == BIOME_PLAINS || b == BIOME_TAIGA || b == BIOME_EXTREME_HILLS || b == BIOME_SAVANNA || b == BIOME_SWAMP))
        return false;
...
static void buildChunkVerticesCPU(int cx, int cz, std::vector<float> &outVerts)
{
    ...
    for(int lx = 0; lx < 16; lx++){
        for(int lz = 0; lz < 16; lz++){
            ...
            Biome bBase = getBiomeBase(wx, wz);
            Biome b = getBiomeAt(wx, wz);

            int surfaceY = (bBase == BIOME_EXTREME_HILLS)
                ? getExtremeHillsBase(wx, wz)
                : getHeight2D(wx, wz, bBase);
